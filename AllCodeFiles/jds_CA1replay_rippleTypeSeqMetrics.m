function jds_CA1replay_rippleTypeSeqMetrics(animalprefixlist)
...
proprev_noncoord = [];
proprev_coord = [];
jd_idx_noncoord = zeros(1,105);
jd_idx_coord = zeros(1,105);
...
%                         if jDist/pmatlength1 > 0.3
                            jd_idx_noncoord(I2) = jd_idx_noncoord(I2) + 1;
%                         end
                    end
                end
            ...
            for ii = 1:length(sigcoordIdx3)
                ...
                for j = 1:size(pmat2,2)-1
                    ...
                    if (sum(tmp) ~= 0) && (sum(tmp2) ~= 0)
                        ...
                        jDist = abs(I1-I2)*2;
                        jDistBins = [jDistBins; jDist];
%                         if jDist/pmatlength2 > 0.3
                            jd_idx_coord(I2) = jd_idx_coord(I2) + 1;
%                         end
                    end
                end
                ...
                maxjump = max(jDistBins);
                jd2 = maxjump/pmatlength2;
                jdist_coord = [jdist_coord; jd2];
                pval_coord = [pval_coord; pv2];
                wc_coord = [wc_coord; abs(weighted_corr)];
...
figure
plot(jd_idx_noncoord./sum(jd_idx_noncoord)); hold on;
plot(jd_idx_coord./sum(jd_idx_coord))
ylabel('Jump to location probability')
xlabel('Position bin')
