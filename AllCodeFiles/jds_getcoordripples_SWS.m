function jds_getcoordripples_SWS(animalprefixlist, eps, area)
...
for a = 1:length(animalprefixlist)
    ...
    for ep = eps
        ...
        if isequal(area,'CA1')
            rip = ripplecoordination_ca1{ep}.coordinated;
        elseif isequal(area,'Ctx')
            rip = ripplecoordination_ctx{ep}.coordinated;
        end
    ...
    if (savedata == 1) && (isequal(area,'CA1'))
        ripple = ripplenew;
        save(sprintf('%s/%srippletime_coordSWS%02d.mat', dir, animalprefix, day), 'ripple');
    elseif (savedata == 1) && (isequal(area,'Ctx'))
        ctxripple = ripplenew;
        save(sprintf('%s/%sctxrippletime_coordSWS%02d.mat', dir, animalprefix, day), 'ctxripple');
    end
end
